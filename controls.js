...
var sound = new Howl({
    urls: ['music/Monkey-Island-Band.ogg'],
    loop: true
});
var touchSound = new Howl({
    urls: ['music/click.ogg'],
    loop: false,
    volume: 0.8
});
var smashSound = new Howl({
    urls: ['music/smash.ogg'],
    loop: false,
    volume: 0.5
});
var tutorialTween;
...
function GameControl(io) {
    ...
    this.onResize = function(event) {
        ...
        /*
        console.log('Physical W/H = ' + (window.innerWidth / ORGINAL_PIXEL_RATIO) + '/' + (window.innerHeight / ORGINAL_PIXEL_RATIO));
        ...
        console.log('pixel_ratio = ' + ORGINAL_PIXEL_RATIO);
        */
...
function resizeScreen(io) {
    ...
    if (device != "mobile") {
        ...
        io.canvas.style.width = io.canvas.width + 'px';
        io.canvas.style.height = io.canvas.height + 'px';
...
function intro(io) {
    ...
    io.addToGroup('BACKGROUND', new iio.Rect(pxConv(GAMEWIDTH / 2), pxConv(GAMEHEIGHT / 2), pxConv(GAMEWIDTH), pxConv(GAMEHEIGHT)).addImage('img/mountain.png', function() {
        console.log('bgLoad');
        loadResources++
    }), -30);
    ...
    startBtn = io.addToGroup('MENU', new iio.Rect(io.canvas.width / 2, -100, pxConv(98), pxConv(94))
        .addImage('img/startBtn.png', function() {
            console.log('startBtnLoad');
            loadResources++
        }), 20);
    ...
    new TWEEN.Tween({
        ...
        .to({
            x: io.canvas.width / 2,
            y: io.canvas.height / 2 - logo.height / 2
        }, 1000)
        .easing(TWEEN.Easing.Elastic.Out)
        .onUpdate(function() {
            logo.pos.y = this.y;
    ...
    muteBtn = io.addToGroup('MENU', new iio.Rect(io.canvas.width - pxConv(30), pxConv(30), pxConv(30), pxConv(30))
        .setRoundingRadius(pxConv(2))
        .setFillStyle(colors['orange'][0])
        .setStrokeStyle(colors['orange'][1], pxConv(2)), 20);
...
function tutorial(io) {
    ...
    tutorialTween = new TWEEN.Tween({
            x: finger.pos.x,
            y: finger.pos.y
        })
        .to({
            x: destinationBlock.x,
            y: destinationBlock.y
        }, 2000)
            ...
            new TWEEN.Tween({
                ...
                .onComplete(function() {
                    new TWEEN.Tween({
                        ...
                        .to({
                            fx: destinationGoal.x,
                            fy: destinationGoal.y,
                            jx: destinationGoal.x,
                            jy: destinationGoal.y
                        }, 1500)
                        .easing(TWEEN.Easing.Quadratic.In)
                        .onUpdate(function() {
...
function endCredits(io) {
    ...
    setTimeout(function() {
        ...
        prepShape(bodyDef, fixDef).addImage('img/credits-name.png');
    }, 0);
    setTimeout(function() {
        bodyDef.angle = -0.5;
        fixDef.shape.SetAsBox(pxConv(120, true), pxConv(25, true));
        bodyDef.position.Set(pxConv(GAMEWIDTH / 2, true), pxConv(-60, true));
        prepShape(bodyDef, fixDef).addImage('img/credits-by.png');
    }, 100);
    setTimeout(function() {
        bodyDef.angle = 0;
        fixDef.shape.SetAsBox(pxConv(140, true), pxConv(35, true));
        bodyDef.position.Set(pxConv(GAMEWIDTH / 2, true), pxConv(-160, true));
        prepShape(bodyDef, fixDef).addImage('img/credits-playing.png');
    }, 1300);
    setTimeout(function() {
        fixDef.shape.SetAsBox(pxConv(50, true), pxConv(25, true));
        bodyDef.position.Set(pxConv(GAMEWIDTH / 2, true), pxConv(-200, true));
        prepShape(bodyDef, fixDef).addImage('img/credits-for.png');
    }, 1010);
    setTimeout(function() {
        fixDef.shape.SetAsBox(pxConv(150, true), pxConv(35, true));
        bodyDef.position.Set(pxConv(GAMEWIDTH / 2, true), pxConv(-260, true));
        prepShape(bodyDef, fixDef).addImage('img/credits-thanks.png');
    }, 1200);
    setTimeout(function() {
        fixDef.shape.SetAsBox(pxConv(150, true), pxConv(100, true));
